...
void init_proc()
{
	...
	for(int i=0; i< RQ_NQS; i++)
		list_init(&runq[i]);//초기화

	list_push_back(&plist, &cur_process->elem_all);
	//list_push_back(&rlist, &cur_process->elem_stat);
	//elem_all이랑 elem_stat차이
	list_push_back(&runq[24], &cur_process->elem_stat);
}
...
pid_t getValidPid(int *idx) {
	...
	while(lock_pid_simple); //0이아니면0 무한루프 돌면서 lock
...
void recalculate_priority(void)
{
	...
	p -> priority += (p -> time_slice) / 10; //스케줄링 된 이후 매 30tick 마다 CPU 사용량에 따라 priority 값을 재계산함. 수정해야데!!
	}
	list_remove(&p->elem_stat);
	proc_getpos(p->priority, &idx);
	list_push_back(&runq[idx], &cur_process->elem_stat); //해당 인덱스의 마지막에 삽입
...
void proc_sleep(unsigned ticks)
{
	...
	/* You should modify this function... *///from here hasnt modified yet
...
void kernel3_proc(void* aux)
{
	...
	{
		/* Your code goes here... */
			if(cur_process->time_used == 50) {
					proc_sleep(60);
			}
			else if(cur_process->time_used == 100) {
					proc_sleep(60);
			}
