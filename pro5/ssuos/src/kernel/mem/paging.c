...
void init_paging() //page table 을 2MB 까지 초기화함.
{
	uint32_t *page_dir = palloc_get_page();//obtains a single free page and returns its address.
	uint32_t *page_tbl = palloc_get_page();
	page_dir = VH_TO_RH(page_dir); //실제 메모리내  주소
