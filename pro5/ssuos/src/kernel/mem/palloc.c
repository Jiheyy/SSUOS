...
palloc_get_multiple (size_t page_cnt)
{
	...
	while(khpage != NULL){
		if(khpage->nalloc == page_cnt){
			...
			/***get key, value, idx for hashtable***/
			...
			if(prepage == khpage){
				...
				result = insert_hash_table(pages, key);
				freelist.nfree--;
				break;
			}
	...
	{
		key = page_idx;
	//	del_hash_table(pages, key);
		memset (pages, 0, PAGE_SIZE * page_cnt);
		if(result == 0)
			insert_hash_table(pages, key);
	}
