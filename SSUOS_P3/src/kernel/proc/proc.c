...
void test1_proc(void *aux)
{
	...
	if((flag = fcntl(fd, F_GETFL, NULL)) == -1)
		return;

	if(~flag & O_APPEND)
		printk("test file has not O_APPEND flag\n");

	write(fd, "oslab", 5);
	read(fd, buf, sizeof(buf));
	printk("Before O_APPEND FLAG : %s\n", buf);
	...
	if(flag & O_APPEND)
		printk("test file has O_APPEND flag\n");

	write(fd,"hellooslab", 10);

	read(fd, buf, sizeof(buf));
	printk("After O_APPEND FLAG : %s\n", buf);
}

void test2_proc(void *aux)
{
	char buf[BUFSIZE] = {0};
	
	int fd, flag;
	int fcntl_fd1, fcntl_fd2;

	if((fd = open("test", O_TRUNC | O_RDWR)) == -1)
		return ;
	...
	if(flag & O_TRUNC)
		printk("test file has O_TRUNC flag\n");


	write(fd, "hello", 5);
	read(fd, buf, sizeof(buf));

	printk("before fcntl F_DUPFD : %s\n", buf);

	if((fcntl_fd1 = fcntl(fd, F_DUPFD, 2)) == -1)
		return;

	printk("First flag : %d\n", fcntl_fd1);

	if((fcntl_fd2 = fcntl(fd, F_DUPFD, 2)) == -1)
		return;

	printk("Second flag : %d\n", fcntl_fd2);

	write(fcntl_fd2, "oslab", 5);
	read(fcntl_fd2, buf, sizeof(buf));

	printk("after fcntl F_DUPFD : %s\n", buf);
}
/**********************************************************/
...
void login_proc(void* aux)
{
	...
	while(1)
	{
		...
		if(strncmp(id, buf2, 5) != 0)
			continue;
		
		if(strncmp(password, buf2+6, 5) != 0){
			continue;
		}
