...
int inode_write(struct inode * in, uint32_t offset, char * buf, int len)
{
	...
	uint32_t blkoff = offset / SSU_BLOCK_SIZE; //184/10 = 18
	uint32_t res_off = offset % SSU_BLOCK_SIZE; // 4

	if(offset > in->sn_size)
		return -1;

	memset(tmpblock, 0, SSU_BLOCK_SIZE);
	if(res_off != 0 || blkoff < in->sn_nlink) // 10 < 11
	{
		fs_readblock(fs, in->sn_directblock[blkoff], tmpblock); //얘는 offset을 이동을 시킨 후에 읽어줘야하는 아이니까
	}
	else
	{
